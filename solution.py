...
def build_packet():
    ...
    # Header creation with mychecksum
    header = struct.pack("bbHHh", ICMP_ECHO_REQUEST, 0, myChecksum, myID, 1)
    # Data unpack
    data = struct.pack("d", sendTime)

    # Generate checksumn on header & data
    myChecksum = checksum(header + data)

    # From Pinger Program: Get correct checksum
    if sys.platform == 'darwin':
       # Convert a 16 bit postive integer from host byte order to network byte order
    ...
    for ttl in range(1,MAX_HOPS):
        for tries in range(TRIES):
            ...
            try:
                ...
                if timeLeft <= 0:
                    tracelist1.append("* * * Request timed out.")
                    tracelist2.append([str(t), tracelist1[-1]])
            except timeout:
                continue

            else:
                #Fill in start
                #Fetch the icmp type from the IP packe
                # Unpack the whole ICMP header
